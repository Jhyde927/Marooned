...
// 0.0 = closed slit, 1.0 = fully open
uniform float u_openAmount;
// -------------------------------------
...
void main() {
    ...
    // ---- Portal wink open/close ----
    float open = smoothstep(0.0, 1.0, u_openAmount);

    // Show only a thin horizontal band when closed,
    // then expand vertically into the full oval.
    float centerDistY = abs(uv.y - 0.5);
    float visibleHalfHeight = max(open * 0.5, 0.001);

    if (centerDistY > visibleHalfHeight)
        discard;

    // Stretch the visible strip back into full UV space.
    // This makes the portal "wink" open instead of simply fading in.
    uv.y = 0.5 + (uv.y - 0.5) / max(open, 0.001);
    // ---------------------------------
